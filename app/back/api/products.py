...
async def update_product(
    ...
    if not response:
        raise HTTPException(
            status_code=404, detail="Product not found in the Wildberries"
        )
    wb_product = response[0]
    ...
    product = await update_product_by_artikul(
        session, artikul, **product.dict(exclude_unset=True)
    )
    if not product:
        raise HTTPException(status_code=404, detail="Product not found")
    return product
...
@router.get("/details/{artikul}", response_model=ProductOut)
